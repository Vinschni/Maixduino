...
void Sipeed_ST7789::drawImage(uint16_t x1, uint16_t y1, uint16_t width, uint16_t height, void* img, bool gray)
{
    configASSERT(img != nullptr || img != 0);

    if (!gray)
    {
        lcd_draw_picture(x1, y1, width, height, (uint16_t *)img);
    }
    else
    {
        lcd_draw_pic_gray(x1, y1, width, height,(uint8_t *) img);
    }
}
